...
export async function addTime(time: string, userID: number, gameName: string, trackName: string, carName: string, weatherName: string, tyreName: string, setup: AT_CarSetupEntry, valid: boolean): Promise<boolean> {
    return new Promise((resolve, reject) => {
        ...
        getDBConnection(sql)
        .then(conn => {
            const query = "EXECUTE AddAutoTime @Time, @UserID, @GameName, @TrackName, @CarName, @WeatherName, @TyreName, "
            ...
            req.addParameter('SetupFrontCamber', sql.TYPES.Float, setup.FrontCamber);
            ...
            req.addParameter('SetupFrontSuspension', sql.TYPES.Int, setup.FrontSuspension);
            req.addParameter('SetupRearSuspension', sql.TYPES.Int, setup.RearSuspension);
            req.addParameter('SetupFrontAntiRollBar', sql.TYPES.Int, setup.FrontAntiRollBar);
            req.addParameter('SetupRearAntiRollBar', sql.TYPES.Int, setup.RearAntiRollBar);
            req.addParameter('SetupFrontSuspensionHeight', sql.TYPES.Int, setup.FrontSuspensionHeight);
            req.addParameter('SetupRearSuspensionHeight', sql.TYPES.Int, setup.RearSuspensionHeight);
            req.addParameter('SetupBrakePressure', sql.TYPES.Int, setup.BrakePressure);
            req.addParameter('SetupBrakeBias', sql.TYPES.Int, setup.BrakeBias);
            req.addParameter('SetupRearLeftTyrePressure', sql.TYPES.Float, setup.RearLeftTyrePressure);
            req.addParameter('SetupRearRightTyrePressure', sql.TYPES.Float, setup.RearRightTyrePressure);
            req.addParameter('SetupFrontLeftTyrePressure', sql.TYPES.Float, setup.FrontLeftTyrePressure);
            req.addParameter('SetupFrontRightTyrePressure', sql.TYPES.Float, setup.FrontRightTyrePressure);

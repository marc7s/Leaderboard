...
import {
  TimeSummary,
  LapRecordType,
} from '../../Backend/src/shared/dataStructures';
...
import {
  ATE_BrakingAssist,
  ATE_DRSAssist,
  ATE_DynamicRacingLine,
  ATE_DynamicRacingLineType,
  ATE_ERSAssist,
  ATE_ForecastAccuracy,
  ATE_FormulaType,
  ATE_GearboxAssist,
  ATE_NetworkGame,
  ATE_PitAssist,
  ATE_PitReleaseAssist,
  ATE_SafetyCarStatus,
  ATE_SessionType,
  ATE_SteeringAssist,
  ATE_Track,
  ...
  AT_Session,
  _AT_SESSION_MAP_,
} from './specification/sessionTypes';
import {
  ATE_ActualTyre,
  ATE_VisualTyre,
  AT_LapHistoryDataEntry,
  AT_SessionHistory,
  AT_TyreStintDataEntry,
  GetLapFlags as getLapFlags,
} from './specification/sessionHistoryTypes';
import {
  _AT_LAP_HISTORY_DATA_ENTRY_MAP_,
  _AT_SESSION_HISTORY_MAP_,
  _AT_TYRE_STINT_DATA_ENTRY_MAP_,
} from './specification/sessionHistoryTypes';
import {
  ATE_Driver,
  ATE_Nationality,
  ...
  ATE_YourTelemetry,
  AT_ParticipantData,
  _AT_PARTICIPANTS_MAP_,
  _AT_PARTICIPANT_DATA_MAP_,
} from './specification/participantTypes';
import {
  AT_CarSetupEntry,
  CompareCarSetups,
  PresetCarSetups,
  _AT_CAR_SETUP_ENTRY_MAP_,
} from './specification/carSetupsTypes';
...
class ReadyStatus {
  ...
  private static receivedTelemetry: boolean = false;
  private static receivedDriver: boolean = false;

  private static telemetryTimeoutMS: number = 5000;
  private static lastTelemetryReceived: number = 0;
  ...
  private static CheckStatus(): void {
    ...
    if (!ReadyStatus.receivedTelemetry) missingStatus.push('Telemetry');
    if (!ReadyStatus.receivedDriver) missingStatus.push('Driver');

    if (missingStatus.length < 1) {
      ReadyStatus.ready = true;
      const status: string = 'Ready! Waiting for a lap time...';
      ReadyStatus.currentStatus = status;
      log(status);
      return;
    }

    const status = `Not ready: Waiting for ${missingStatus.join(' and ')}.`;
    if (status !== ReadyStatus.currentStatus) {
      ReadyStatus.currentStatus = status;
      log(status);
    }
  }

  private static TimeoutTelemetryReceived(): void {
    if (
      Date.now() - ReadyStatus.lastTelemetryReceived >
      ReadyStatus.telemetryTimeoutMS
    ) {
      ReadyStatus.receivedTelemetry = false;
  ...
  public static Initialize(): void {
    ...
    // Require continous telemetry data to be received in order to stay ready
    setInterval(
      ReadyStatus.TimeoutTelemetryReceived,
      ReadyStatus.telemetryTimeoutMS
    );
  }

  public static SetTelemetryReceived(): void {
    ReadyStatus.receivedTelemetry = true;
    ReadyStatus.lastTelemetryReceived = Date.now();
    ReadyStatus.CheckStatus();
  }

  public static SetDriverReceived(): void {
    ReadyStatus.receivedDriver = true;
    ReadyStatus.CheckStatus();
  }
}
...
export class Cache {
  ...
  private static cachePath: string = Cache.folderPath + 'addedTimes.json';
  // Cache values for 24 hours
  private static cacheTimeMilliSeconds: number = 1000 * 60 * 60 * 24;

  private static AddedTimes: CacheEntry[] = [];
  ...
  private static WriteToCache(times: CacheEntry[]) {
    fs.writeFileSync(this.cachePath, JSON.stringify(times));
  }

  private static _IncludesTime(timeID: string, updateCache: boolean): boolean {
    if (updateCache) Cache.UpdateTimeCache();
...
function main() {
  const client = new F1TelemetryClient({
    port: process.env.TELEMETRY_UDP_PORT as any as number,
  });

  Cache.Initialize();

  client.on(PACKETS.session, (e) => parseHeader(e, handleSessionData));
  client.on(PACKETS.sessionHistory, (e) =>
    parseHeader(e, handleSessionHistoryData)
  );
  client.on(PACKETS.participants, (e) =>
    parseHeader(e, handleParticipantsData)
  );
...
app.use((err: any, req: Request, res: Response, next: NextFunction) => {
  if (err) {
    ...
    switch (err.message) {
      case 'NoUserIDProvided':
        return sendError(res, 400, 'Error: Missing user ID', requestIP);
      default:
        return sendError(res, 500, err.message, requestIP);
    }
  } else {
    next();
  }
...
function sendError(
  ...
  if (!errorDetails) return res.status(statusCode).send(payload);
...
function parseHeader(event: any, handler: Function): void {
  const header: AT_Header = parseWithMapping(
    event['m_header'],
    _AT_HEADER_MAP_
  );
...
function handleSessionData(header: AT_Header, data: any): void {
  ReadyStatus.SetTelemetryReceived();
  ...
  session.SafetyCarStatus = parseEnum(
    session.SafetyCarStatusID,
    ATE_SafetyCarStatus
  );
  session.NetworkGame = parseEnum(session.NetworkGameID, ATE_NetworkGame);
  session.ForecastAccuracy = parseEnum(
    session.ForecastAccuracyID,
    ATE_ForecastAccuracy
  );
  session.SteeringAssist = parseEnum(
    session.SteeringAssistID,
    ATE_SteeringAssist
  );
  ...
  session.PitReleaseAssist = parseEnum(
    session.PitReleaseAssistID,
    ATE_PitReleaseAssist
  );
  ...
  session.DynamicRacingLine = parseEnum(
    session.DynamicRacingLineID,
    ATE_DynamicRacingLine
  );
  session.DynamicRacingLineType = parseEnum(
    session.DynamicRacingLineTypeID,
    ATE_DynamicRacingLineType
  );
...
function handleSessionHistoryData(header: AT_Header, data: any): void {
  ...
  if (header.PlayerCarIndex == data['m_carIdx']) {
    ...
    const sessionHistory: AT_SessionHistory = parseWithMapping(
      data,
      _AT_SESSION_HISTORY_MAP_
    );
    ...
    const rawLapHistoryData =
      rawLapHistoryDataList[Math.max(sessionHistory.NumLaps - 2, 0)];
    const lapHistoryEntry: AT_LapHistoryDataEntry = parseWithMapping(
      rawLapHistoryData,
      _AT_LAP_HISTORY_DATA_ENTRY_MAP_
    );

    // Skip unset times
    if (lapHistoryEntry.LapTimeInMS === 0) return;

    // Parse the time data
    lapHistoryEntry.LapFlags = getLapFlags(
      rawLapHistoryData['m_lapValidBitFlags']
    );
    lapHistoryEntry.LapTimeFormatted = formatLapTime(
      lapHistoryEntry.LapTimeInMS
    );
    ...
    // Parse the tyre data
    const rawTyreStintData =
      rawTyreStintDataList[Math.min(sessionHistory.NumTyreStints - 1, 0)];
    const tyreStintEntry: AT_TyreStintDataEntry = parseWithMapping(
      rawTyreStintData,
      _AT_TYRE_STINT_DATA_ENTRY_MAP_
    );
    tyreStintEntry.TyreActualCompound = parseEnum(
      tyreStintEntry.TyreActualCompoundID,
      ATE_ActualTyre
    );
    tyreStintEntry.TyreVisualCompound = parseEnum(
      tyreStintEntry.TyreVisualCompoundID,
      ATE_VisualTyre
    );
    const tyre: ATE_VisualTyre = tyreStintEntry.TyreVisualCompound;

    // Update the current session history data
    currentLap.SessionHistory = !timeSet
      ? undefined
      : {
          Timestamp: Date.now(),
          TimeSet: true,
...
function handleParticipantsData(header: AT_Header, data: any): void {
  ...
  const participantEntry: AT_ParticipantData = parseWithMapping(
    rawParticipantData,
    _AT_PARTICIPANT_DATA_MAP_
  );
  ...
  participantEntry.Nationality = parseEnum(
    participantEntry.NationalityID,
    ATE_Nationality
  );
  participantEntry.YourTelemetry = parseEnum(
    participantEntry.YourTelemetryID,
    ATE_YourTelemetry
  );
...
function handleCarSetupsData(header: AT_Header, data: any): void {
  ...
  const carSetupEntry: AT_CarSetupEntry = parseWithMapping(
    rawCarSetupsData,
    _AT_CAR_SETUP_ENTRY_MAP_
  );
  ...
  currentLap.CarSetup = {
    ...
    Setup: carSetupEntry,
    SetupName:
      PresetCarSetups.find((x) => CompareCarSetups(x, carSetupEntry))
        ?.SetupName ?? 'Custom',
  };
  checkCurrentTime();
}

function lapTimeToTimeSummary(
  lapTime: LapTime,
  carName: string,
  weatherName: string
): TimeSummary {
...
function checkCurrentTime(): void {
  if (
    currentDriver != null &&
    currentLap.Header != null &&
    currentLap.Session != null &&
    currentLap.SessionHistory != null &&
    currentLap.Participant != null &&
    currentLap.CarSetup != null
  ) {
    const timestamps: number[] = [
      currentLap.Session.Timestamp,
      currentLap.SessionHistory.Timestamp,
      currentLap.Participant.Timestamp,
      currentLap.CarSetup.Timestamp,
    ];

    // Make sure all gathered data is from the same time
    if (maxDifference(timestamps) > maximumAllowedTimestampDifference) return;
    ...
    if (
      lapTimes.find(
        (lapTime) => lapTime.ID === currentLap.SessionHistory?.TimeID
      ) != null
    )
      return;
    ...
    if (
      ...
      weatherName != null &&
      tyreName != null &&
      !Cache.IncludesTime(currTime.ID)
    ) {
      const timeSummary: TimeSummary = lapTimeToTimeSummary(
        currTime,
        carName,
        weatherName
      );
      ...
      addTime(
        ...
        .catch((err) => {
          if (
            ...
            err.message === 'Time already exists'
          ) {
